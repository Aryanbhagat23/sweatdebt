...
function NavBar({ user, livePhoto, unreadDMs, onProfileOpen }) {
  ...
  return (
    <nav style={{
      ...
      paddingTop:"10px",paddingBottom:"env(safe-area-inset-bottom,0)",zIndex:1000
    }}>
      {[
...
function AppContent({ user, needsOnboarding, onOnboardingComplete }) {
  ...
  useEffect(()=>{
    ...
    const u=onSnapshot(query(collection(db,"conversations"),where("participants","array-contains",user.uid)),snap=>{
      let n=0;snap.docs.forEach(d=>{n+=d.data().unreadCount?.[user.uid]||0;});setUnreadDMs(n);
    },()=>{});
    return()=>u();
  },[user]);
...
export default function App() {
  ...
  useEffect(()=>{document.body.style.background=T.bg0;document.body.style.color=T.textDark;},[]);
