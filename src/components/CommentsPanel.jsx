...
export default function CommentsPanel({ videoId, currentUser, onCountChange, onClose }) {
  ...
  useEffect(() => {
    const u = onSnapshot(query(collection(db,"videos",videoId,"comments")), snap=>{
      ...
      onCountChange(data.length); setLoading(false);
    });
    return ()=>u();
  }, [videoId]);

  useEffect(()=>{ if(replyTo) setTimeout(()=>inputRef.current?.focus(),100); },[replyTo]);
  useEffect(()=>{ if(!loading) setTimeout(()=>listRef.current?.scrollTo({top:99999,behavior:"smooth"}),100); },[all,loading]);

  const post = async() => {
    if(!text.trim()||posting)return;
    setPosting(true); const t=text.trim(); setText(""); setReplyTo(null);
    try {
      await addDoc(collection(db,"videos",videoId,"comments"),{text:t,userId:currentUser.uid,userName:currentUser.displayName,userPhoto:currentUser.photoURL||null,createdAt:serverTimestamp(),likes:0,replyTo:replyTo?.id||null,replyToName:replyTo?.userName||null});
      await updateDoc(doc(db,"videos",videoId),{comments:increment(1)});
    }catch(e){console.error(e);setText(t);}
    setPosting(false);
  };

  const timeAgo=ts=>{if(!ts?.toDate)return"now";const s=Math.floor((new Date()-ts.toDate())/1000);if(s<60)return"now";if(s<3600)return`${Math.floor(s/60)}m`;if(s<86400)return`${Math.floor(s/3600)}h`;return`${Math.floor(s/86400)}d`;};
  const REACTIONS=["😂","🔥","💀","😤","👑","🫡","💪","😭"];
  ...
  return (
    <>
      ...
      <div style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",zIndex:2000 }} onClick={onClose}/>
      <div style={{ position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:"480px",height:"75vh",background:T.bg1,borderRadius:"20px 20px 0 0",zIndex:2001,animation:"slideUpPanel 0.35s cubic-bezier(0.32,0.72,0,1)",display:"flex",flexDirection:"column" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 16px 8px",flexShrink:0 }}>
          ...
          <div style={{ fontSize:"18px",color:T.muted,cursor:"pointer",width:"44px",height:"44px",display:"flex",alignItems:"center",justifyContent:"center" }} onClick={onClose}>✕</div>
        </div>
        <div style={{ display:"flex",gap:"8px",padding:"0 16px 10px",overflowX:"auto",flexShrink:0 }}>
          {REACTIONS.map(e=>(
            <button key={e} style={{ background:T.bg2,border:`1px solid ${T.border}`,borderRadius:T.rFull,padding:"6px 12px",fontSize:"18px",cursor:"pointer",flexShrink:0 }} onClick={()=>{setText(p=>p+e);inputRef.current?.focus();}}>{e}</button>
          ))}
        </div>
          ...
          {loading?(
            ...
            all.map(c=>(
              <div key={c.id} style={{ marginBottom:"4px" }}>
                <CommentRow c={c} isOwn={c.userId===currentUser?.uid} timeAgo={timeAgo} onReply={()=>{setReplyTo({id:c.id,userName:c.userName});setText("");setTimeout(()=>inputRef.current?.focus(),100);}} onProfile={()=>{onClose();navigate(`/profile/${c.userId}`);}} replyCount={c.replies?.length||0} showReplies={true}/>
                {c.replies?.length>0&&(
                  <div style={{ marginLeft:"44px",borderLeft:`2px solid ${T.border}`,paddingLeft:"12px",marginBottom:"8px" }}>
                    {c.replies.map(r=><CommentRow key={r.id} c={r} isOwn={r.userId===currentUser?.uid} timeAgo={timeAgo} onReply={()=>{setReplyTo({id:c.id,userName:c.userName});setText("");setTimeout(()=>inputRef.current?.focus(),100);}} onProfile={()=>{onClose();navigate(`/profile/${r.userId}`);}} isReply/>)}
                  </div>
                )}
              </div>
            ))
        ...
        {replyTo&&(
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 16px",background:T.bg2,borderTop:`1px solid ${T.border}`,flexShrink:0 }}>
            ...
            <div style={{ color:T.muted,cursor:"pointer",fontSize:"16px",padding:"4px 8px" }} onClick={()=>{setReplyTo(null);setText("");}}>✕</div>
          </div>
        )}
        <div style={{ flexShrink:0,background:T.bg1,borderTop:`1px solid ${T.border}`,padding:"12px 16px",paddingBottom:"calc(16px + env(safe-area-inset-bottom,0px))" }}>
          <div style={{ display:"flex",alignItems:"center",gap:"10px" }}>
            {currentUser?.photoURL?<img src={currentUser.photoURL} alt="" style={{ width:"34px",height:"34px",borderRadius:"50%",objectFit:"cover",flexShrink:0 }}/>
            ...
            <div style={{ flex:1,display:"flex",alignItems:"center",gap:"8px",background:T.bg3,border:`1px solid ${T.borderMid}`,borderRadius:T.rFull,padding:"10px 14px" }}>
              <input ref={inputRef} value={text} onChange={e=>setText(e.target.value)} placeholder={replyTo?`Reply to @${replyTo.userName}...`:"Add a comment..."} onKeyDown={e=>e.key==="Enter"&&post()} maxLength={300}
              ...
              {text.trim().length>0&&<button style={{ background:T.gradPrimary,border:"none",borderRadius:"50%",width:"30px",height:"30px",fontSize:"14px",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",opacity:posting?0.5:1,flexShrink:0 }} onClick={post} disabled={posting}>{posting?"…":"↑"}</button>}
            </div>
          </div>
        </div>
      </div>
...
function CommentRow({ c, isOwn, timeAgo, onReply, onProfile, isReply, replyCount, showReplies }) {
  ...
  return (
    <div style={{ display:"flex",gap:"10px",padding:"10px 0",borderBottom:`1px solid ${T.bg3}` }}>
      <div style={{ cursor:"pointer",flexShrink:0 }} onClick={onProfile}>
        {c.userPhoto?<img src={c.userPhoto} alt="" style={{ width:sz,height:sz,borderRadius:"50%",objectFit:"cover" }}/>
