    ...
    <div style={{ minHeight:"100vh", background:T.bg0 }}>
      ...
      {vids.length === 0 ? (
        <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:"16px", padding:"24px" }}>
          ...
          {tab==="friends" && (
            <button style={{ background:T.pinkDim, border:`1px solid ${T.pinkBorder}`, borderRadius:T.r16, padding:"12px 20px", cursor:"pointer", fontFamily:T.fontDisplay, fontSize:"18px", color:T.pink, letterSpacing:"0.04em" }}
...
function ReelsFeed({ videos, user, navigate, onBadge }) {
  ...
  // Reset to top only when tab changes (videos array identity changes)
  // Do NOT reset when videos update (realtime updates)
  const prevLen = useRef(videos.length);
  useEffect(() => {
    if (prevLen.current === 0 && videos.length > 0) setIdx(0);
    prevLen.current = videos.length;
  }, [videos]);

  // Touch handlers — fixed to allow going back
  ...
  const handleTouchEnd = e => {
    if (startY.current === null || commentsOpen) return;
    const dy = startY.current - e.changedTouches[0].clientY;
    const dx = Math.abs(e.changedTouches[0].clientX - (startX.current || 0));
    // Only trigger if vertical swipe (not horizontal)
    if (Math.abs(dy) < 60 || dx > 40) { startY.current = null; return; }
    const now = Date.now();
    if (now - last.current < 400) { startY.current = null; return; }
    last.current = now;
    // Swipe up = next video, swipe down = previous video
    if (dy > 0 && idx < videos.length - 1) setIdx(p => p + 1);
    else if (dy < 0 && idx > 0) setIdx(p => p - 1);
    startY.current = null;
  };
  ...
  const handleWheel = useCallback(e => {
    ...
    if (e.deltaY > 40 && idx < videos.length - 1) { last.current = now; setIdx(p => p + 1); }
    else if (e.deltaY < -40 && idx > 0) { last.current = now; setIdx(p => p - 1); }
  }, [idx, videos.length, commentsOpen]);
    ...
    <div ref={ref} style={{ height:"100vh", overflow:"hidden", position:"relative" }}
      ...
      <div style={{ position:"fixed", right:"8px", top:"50%", transform:"translateY(-50%)", display:"flex", flexDirection:"column", gap:"5px", zIndex:50 }}>
        {videos.slice(Math.max(0,idx-4), idx+5).map((_,i) => {
          ...
          return <div key={a} style={{ width:"3px", height:a===idx?"18px":"4px", borderRadius:"2px", background:a===idx?T.pink:"rgba(255,255,255,0.15)", transition:"all 0.3s", cursor:"pointer" }} onClick={() => setIdx(a)} />;
        })}
      </div>
    </div>
  );
...
function ReelCard({ video, user, isActive, offset, navigate, onBadge, onCommentsChange }) {
  ...
  // Tell parent when comments open/close so swipe is disabled
  useEffect(() => {
    onCommentsChange(showComments);
  }, [showComments]);
  ...
  useEffect(() => {
    ...
    if (isActive) { v.play().catch(() => {}); }
      ...
      v.pause();
      v.currentTime = 0;
      // Don't close comments here — causes jump bug
    }
  }, [isActive]);

  // Close comments when video is no longer active
  useEffect(() => {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      if (video.betCreatedBy) {
        const nb = await recordWin(video.betCreatedBy);
        if (video.betCreatedBy === user?.uid && nb?.length > 0) onBadge(nb[0]);
      }
      if (video.uploadedBy) {
        const nb = await recordLossApproved(video.uploadedBy);
        if (video.uploadedBy === user?.uid && nb?.length > 0) onBadge(nb[0]);
      }
      setApproved(true);
  ...
  return (
    <div style={{ position:"absolute", inset:0, transform:`translateY(${offset*100}%)`, transition:"transform 0.42s cubic-bezier(0.25,0.46,0.45,0.94)", background:T.bg0, overflow:"hidden" }}>
      <video ref={vRef} src={video.videoUrl}
      ...
      {/* RIGHT SIDE ACTIONS — hide when comments open */}
      {!showComments && (
      ...
      {/* Bottom info — hide when comments open */}
      {!showComments && (
      ...
      {/* Comments panel — smooth animation fix */}
      {showComments && (
        <CommentsPanel
          videoId={video.id}
          currentUser={user}
