...
function CommentsSection({ videoId, currentUser, onCountChange }) {
  ...
  const postComment = async () => {
    ...
    try {
      ...
      // Update comment count on video doc
      await updateDoc(doc(db,"videos",videoId),{
        comments: increment(1)
      });
    } catch(e) {
      console.error(e);
      setText(newComment); // restore if failed
    }
    setPosting(false);
  };
    ...
    {[...Array(3)].map((_,i)=>(
      ...
      <div style={CS.inputRow}>
        ...
        <div style={CS.inputWrap}>
          ...
          {text.trim().length > 0 && (
            <button
              style={{...CS.sendBtn,opacity:posting?0.5:1}}
              onClick={postComment}
              disabled={posting}
            >
              {posting?"...":"↑"}
...
function CommentItem({ comment, videoId, currentUser, timeAgo, isLast }) {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap=>setReplies(snap.docs.map(d=>({id:d.id,...d.data()}))));
    return ()=>unsub();
  }, [showReplies]);
  ...
  const likeComment = async () => {
    ...
    setLikes(liked?likes-1:likes+1);
    await updateDoc(
      doc(db,"videos",videoId,"comments",comment.id),
      {likes:increment(liked?-1:1)}
    );
  };
...
const S = {
  ...
  tab:{padding:"12px 16px",fontSize:"15px",fontWeight:"500",color:"#666",cursor:"pointer",transition:"all 0.2s",fontFamily:"'DM Sans',sans-serif"},
  center:{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"50vh"},
  loadingText:{color:"#555",fontSize:"13px",fontFamily:"'DM Mono',monospace"},
  ...
  avatar:{width:"38px",height:"38px",borderRadius:"50%",background:"linear-gradient(135deg,#d4ff00,#ff5c1a)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"15px",fontWeight:"700",color:"#000",fontFamily:"'Bebas Neue',sans-serif"},
...
const CS = {
  ...
  loadingText:{color:"#444",fontFamily:"'DM Mono',monospace",fontSize:"12px",textAlign:"center",padding:"20px"},
  ...
  inputAvatarFallback:{width:"32px",height:"32px",borderRadius:"50%",background:"linear-gradient(135deg,#d4ff00,#ff5c1a)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Bebas Neue',sans-serif",fontSize:"14px",fontWeight:"700",color:"#000",flexShrink:0},
  ...
  sendBtn:{background:"#d4ff00",border:"none",borderRadius:"50%",width:"30px",height:"30px",fontFamily:"'Bebas Neue',sans-serif",fontSize:"16px",fontWeight:"700",color:"#000",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},
  ...
  commentAvatarFallback:{width:"34px",height:"34px",borderRadius:"50%",background:"linear-gradient(135deg,#d4ff00,#ff5c1a)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Bebas Neue',sans-serif",fontSize:"13px",fontWeight:"700",color:"#000",flexShrink:0},
