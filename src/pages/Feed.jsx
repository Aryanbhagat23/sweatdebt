...
function ReelsFeed({ videos, user, navigate, onBadge }) {
  ...
  useEffect(() => {
    ...
    const onMove = e => {
      if (!touch.current.active || commentsRef.current) return;
      const t = e.changedTouches[0];
    ...
    const onEnd = e => {
      if (!touch.current.active || commentsRef.current) { touch.current.active = false; return; }
  ...
  return (
    <div ref={containerRef} style={{ height: "100vh", overflow: "hidden", position: "relative", touchAction: "none", userSelect: "none", WebkitUserSelect: "none" }}>
      {videos.map((v, i) => (
        <ReelCard key={v.id} video={v} user={user} isActive={i === idx} offset={i - idx}
          navigate={navigate} onBadge={onBadge} onCommentsChange={setCommentsOpen} />
      ))}
      {/* Dot nav */}
...
function ReelCard({ video, user, isActive, offset, navigate, onBadge, onCommentsChange }) {
  ...
  // Notify parent when comments open/close (to pause swipe)
  ...
  // Close comments when card goes off screen
  ...
  // ─── THE FIX ─────────────────────────────────────────────────
  // Instead of {showComments && render} or opacity toggling,
  // we use CSS visibility + pointer-events on a SINGLE wrapper div.
  // visibility:hidden removes from interaction but keeps layout intact,
  // so Android never drops the element from the render tree.
  // This means ALL icons come back together when comments close.
  const uiHidden = showComments;
  const uiStyle = {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, transform: `translateY(${offset * 100}%)`, transition: "transform 0.4s cubic-bezier(0.25,0.46,0.45,0.94)", background: T.bg0, overflow: "hidden", touchAction: "none" }}>
      {/* Video */}
      <video ref={vRef} src={video.videoUrl} style={{ width: "100%", height: "100%", objectFit: "cover", position: "absolute", inset: 0 }}
        loop playsInline muted={muted} preload={isActive ? "auto" : "none"} onClick={() => setMuted(!muted)} />
      ...
      {/* ── RIGHT SIDE ICONS ── hidden via visibility (not removed from DOM) */}
      <div style={{ position: "absolute", right: "12px", bottom: "100px", display: "flex", flexDirection: "column", gap: "20px", alignItems: "center", zIndex: 10, ...uiStyle }}>
        {[
      ...
      {/* ── BOTTOM INFO ── hidden via visibility */}
      <div style={{ position: "absolute", bottom: 0, left: 0, right: "60px", padding: "0 16px 90px", zIndex: 10, ...uiStyle }}>
        {/* Uploader info */}
