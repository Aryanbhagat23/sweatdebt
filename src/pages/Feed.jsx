...
export default function Feed({ user, onBellClick }) {
  ...
  useEffect(() => {
    if (!user) return;
    const u = onSnapshot(collection(db,"users",user.uid,"friends"), snap=>setFriendIds(snap.docs.map(d=>d.data().uid).filter(Boolean)), ()=>{});
    return ()=>u();
  }, [user]);
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bg0 }}>
      ...
      <div style={{ position:"fixed", top:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:"480px", zIndex:100, paddingTop:"env(safe-area-inset-top,0)" }}>
        <div style={{ display:"flex", justifyContent:"center", gap:"24px", padding:"14px 20px 10px", background:"rgba(10,10,15,0.96)", backdropFilter:"blur(24px)" }}>
          {[{key:"forYou",label:"For You"},{key:"friends",label:"Friends"},{key:"trending",label:"🔥 Trending"}].map(t=>(
            <div key={t.key} style={{ fontFamily:T.fontBody, fontSize:"15px", fontWeight:"600", color:tab===t.key?T.white:"rgba(255,255,255,0.3)", cursor:"pointer", borderBottom:tab===t.key?`2px solid ${T.pink}`:"2px solid transparent", paddingBottom:"4px", transition:"all 0.2s" }} onClick={()=>setTab(t.key)}>
              {t.label}
            </div>
          ))}
        </div>
...
function ReelsFeed({ videos, user, navigate, onBadge }) {
  ...
  const last = useRef(0);
  useEffect(()=>setIdx(0),[videos]);
  const handleTouchStart = e => { startY.current = e.touches[0].clientY; };
  const handleTouchEnd = e => {
    if (startY.current===null) return;
    const diff = startY.current - e.changedTouches[0].clientY;
    const now = Date.now();
    if (Math.abs(diff)>60 && now-last.current>400) {
      last.current=now;
      if (diff>0 && idx<videos.length-1) setIdx(p=>p+1);
      else if (diff<0 && idx>0) setIdx(p=>p-1);
    }
    startY.current=null;
  };
  const handleWheel = useCallback(e=>{
    e.preventDefault();
    const now=Date.now(); if(now-last.current<600)return;
  ...
  return (
    <div ref={ref} style={{ height:"100vh", overflow:"hidden", position:"relative" }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      {videos.map((v,i)=><ReelCard key={v.id} video={v} user={user} isActive={i===idx} offset={i-idx} navigate={navigate} onBadge={onBadge}/>)}
      <div style={{ position:"fixed", right:"8px", top:"50%", transform:"translateY(-50%)", display:"flex", flexDirection:"column", gap:"5px", zIndex:50 }}>
        {videos.slice(Math.max(0,idx-4),idx+5).map((_,i)=>{const a=Math.max(0,idx-4)+i; return <div key={a} style={{ width:"3px", height:a===idx?"18px":"4px", borderRadius:"2px", background:a===idx?T.pink:"rgba(255,255,255,0.15)", transition:"all 0.3s", cursor:"pointer" }} onClick={()=>setIdx(a)}/>;}) }
      </div>
    </div>
  );
}
...
function ReelCard({ video, user, isActive, offset, navigate, onBadge }) {
  ...
  return (
    <div style={{ position:"absolute",inset:0,transform:`translateY(${offset*100}%)`,transition:"transform 0.45s cubic-bezier(0.25,0.46,0.45,0.94)",background:T.bg0,overflow:"hidden" }}>
      ...
      <div style={{ position:"absolute",bottom:0,left:0,right:"60px",padding:"0 16px 90px",zIndex:10 }}>
        <div style={{ display:"flex",alignItems:"center",gap:"10px",marginBottom:"10px",cursor:"pointer" }} onClick={()=>navigate(`/profile/${video.uploadedBy}`)}>
          {video.uploaderPhoto?<img src={video.uploaderPhoto} alt="" style={{ width:"42px",height:"42px",borderRadius:"50%",objectFit:"cover",border:`2px solid ${T.pink}`,flexShrink:0 }}/>
