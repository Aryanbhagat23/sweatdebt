...
export default function Feed({ user, onBellClick }) {
  ...
  const getTabVideos = () => {
    if (activeTab === "friends") {
      return videos.filter(v => friendIds.includes(v.uploadedBy) || v.uploadedBy === user?.uid);
    }
    if (activeTab === "trending") {
      return [...videos].sort((a, b) =>
        ((b.likes || 0) + (b.comments || 0) * 2) - ((a.likes || 0) + (a.comments || 0) * 2)
      );
    }
    return videos;
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg0 }}>
      ...
      {/* Upload FAB */}
      <div style={{ position: "fixed", bottom: "84px", right: "20px", zIndex: 500, display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" }}>
        <div
          onClick={() => navigate("/upload")}
          style={{ width: "54px", height: "54px", borderRadius: "50%", background: `linear-gradient(135deg,${C.cyan},${C.purple})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "22px", cursor: "pointer", boxShadow: `0 4px 24px rgba(0,212,255,0.45)`, transition: "transform 0.2s" }}
          onMouseDown={e => e.currentTarget.style.transform = "scale(0.9)"}
          onMouseUp={e => e.currentTarget.style.transform = "scale(1)"}
          onTouchStart={e => e.currentTarget.style.transform = "scale(0.9)"}
          onTouchEnd={e => { e.currentTarget.style.transform = "scale(1)"; navigate("/upload"); }}
        >📹</div>
        <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "9px", color: C.muted, letterSpacing: "0.06em" }}>UPLOAD</div>
      </div>
      ...
      {displayVideos.length > 0 && <ReelsFeed videos={displayVideos} user={user} />}
    </div>
  );
}
  ...
  return (
    <div ref={containerRef} style={{ height: "100vh", overflow: "hidden", position: "relative" }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      {videos.map((video, index) => (
        <ReelCard key={video.id} video={video} user={user} isActive={index === currentIndex} offset={index - currentIndex} />
      ))}
...
function ReelCard({ video, user, isActive, offset }) {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      // Winner (bet creator) gets +1 win +5 honour
      if (video.betCreatedBy) {
        await updateDoc(doc(db, "users", video.betCreatedBy), {
          wins: increment(1),
          honour: increment(5),
        });
      }
      // Loser (uploader) gets +1 loss but +3 honour for honest completion
      if (video.uploadedBy) {
        await updateDoc(doc(db, "users", video.uploadedBy), {
          losses: increment(1),
          honour: increment(3),
        });
      }
  ...
  const handleDispute = async () => {
    ...
    try {
      ...
      // Loser's honour drops -15 for not completing forfeit properly
      if (video.uploadedBy) {
        await updateDoc(doc(db, "users", video.uploadedBy), {
          losses: increment(1),
          honour: increment(-15),
    ...
    <>
      ...
      <div style={{ position: "absolute", inset: 0, transform: `translateY(${offset * 100}%)`, transition: "transform 0.45s cubic-bezier(0.25,0.46,0.45,0.94)", background: C.bg0, overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", top: "60px", left: "16px" }}>
          {approved && <div style={{ background: "rgba(0,230,118,0.2)", border: "1px solid rgba(0,230,118,0.6)", color: C.green, padding: "4px 12px", borderRadius: "20px", fontFamily: "'DM Mono',monospace", fontSize: "11px", fontWeight: "700", letterSpacing: "0.06em" }}>APPROVED ✓</div>}
          {disputed && <div style={{ background: "rgba(255,77,109,0.2)", border: "1px solid rgba(255,77,109,0.6)", color: C.red, padding: "4px 12px", borderRadius: "20px", fontFamily: "'DM Mono',monospace", fontSize: "11px", fontWeight: "700", letterSpacing: "0.06em" }}>DISPUTED ✗</div>}
          {!approved && !disputed && <div style={{ background: "rgba(255,107,74,0.2)", border: "1px solid rgba(255,107,74,0.6)", color: C.coral, padding: "4px 12px", borderRadius: "20px", fontFamily: "'DM Mono',monospace", fontSize: "11px", fontWeight: "700", letterSpacing: "0.06em" }}>FORFEIT 💀</div>}
        </div>
        ...
        <div style={{ position: "absolute", right: "12px", bottom: "140px", display: "flex", flexDirection: "column", gap: "20px", alignItems: "center", zIndex: 10 }}>
          {[
            ...
            { icon: "💬", count: commentCount, onClick: () => setShowComments(!showComments) },
            {
              icon: "↗️", label: "Share", onClick: async () => {
                if (navigator.share) await navigator.share({ title: "SweatDebt forfeit", url: window.location.href });
                else navigator.clipboard.writeText(window.location.href);
              }
            },
            { icon: "⚔️", label: "Bet", onClick: () => {} },
          ].map((btn, i) => (
        ...
        <div style={{ position: "absolute", bottom: 0, left: 0, right: "60px", padding: "0 16px 80px", zIndex: 10 }}>
          ...
          {disputed && <div style={{ background: "rgba(255,77,109,0.15)", border: "1px solid rgba(255,77,109,0.4)", borderRadius: "10px", padding: "10px 14px", fontFamily: "'DM Sans',sans-serif", fontSize: "13px", color: C.red, textAlign: "center", marginBottom: "10px" }}>⚠ Disputed — honour score dropped</div>}
        </div>
...
function CommentsPanel({ videoId, currentUser, onCountChange, onClose }) {
  ...
  return (
    <>
      <style>{`@keyframes slideUpPanel{from{transform:translateY(100%)}to{transform:translateY(0)}} @keyframes spin{to{transform:rotate(360deg)}}`}</style>
      <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.55)", zIndex: 200 }} onClick={onClose} />
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: "72vh", background: C.bg1, borderRadius: "20px 20px 0 0", zIndex: 201, animation: "slideUpPanel 0.35s cubic-bezier(0.32,0.72,0,1)", display: "flex", flexDirection: "column", paddingBottom: "env(safe-area-inset-bottom,0)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px 8px", flexShrink: 0 }}>
          ...
          <div style={{ fontSize: "20px", color: C.muted, cursor: "pointer", padding: "4px 8px" }} onClick={onClose}>✕</div>
        </div>
          ...
          {loading ? (
            ...
            <div key={c.id} style={{ display: "flex", gap: "10px", padding: "10px 0", borderBottom: `1px solid ${C.bg3}` }}>
              {c.userPhoto ? (
                ...
                <div style={{ width: "34px", height: "34px", borderRadius: "50%", background: `linear-gradient(135deg,${C.cyan},${C.purple})`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Bebas Neue',sans-serif", fontSize: "13px", color: "#000", flexShrink: 0 }}>
                  {c.userName?.charAt(0) || "?"}
                </div>
              )}
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "14px", color: "rgba(224,242,254,0.9)", lineHeight: "1.5" }}>{c.text}</div>
              </div>
            </div>
          ))}
        </div>

        {/* COMMENT INPUT — text is now visible */}
        <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 16px 16px", borderTop: `1px solid ${C.border1}`, background: C.bg1, flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, display: "flex", alignItems: "center", gap: "8px", background: C.bg3, border: `1.5px solid ${C.border2}`, borderRadius: "24px", padding: "10px 14px" }}>
            <input
              ...
              style={{
                ...
                border: "none",
                outline: "none",
                color: "#e0f2fe",             /* FIXED — bright white text */
                caretColor: "#00d4ff",         /* cyan cursor */
                WebkitTextFillColor: "#e0f2fe", /* Safari fix */
                fontSize: "15px",
