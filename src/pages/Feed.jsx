...
export default function Feed({ user, onBellClick }) {
  ...
  const getVids = () => {
    ...
    if (tab === "trending") return [...videos].sort((a, b) =>
    ...
    <div style={{ minHeight: "100vh", background: T.bg0 }}>
      ...
      <div style={{ position: "fixed", top: 0, left: "50%", transform: "translateX(-50%)", width: "100%", maxWidth: "480px", zIndex: 100, paddingTop: "env(safe-area-inset-top,0)" }}>
        <div style={{ display: "flex", justifyContent: "center", gap: "24px", padding: "14px 20px 10px", background: "rgba(10,10,15,0.97)", backdropFilter: "blur(24px)", WebkitBackdropFilter: "blur(24px)" }}>
          {[
            ...
            <div key={t.key}
              style={{ fontFamily: T.fontBody, fontSize: "15px", fontWeight: "600", color: tab === t.key ? T.white : "rgba(255,255,255,0.3)", cursor: "pointer", borderBottom: tab === t.key ? `2px solid ${T.pink}` : "2px solid transparent", paddingBottom: "4px", transition: "all 0.2s" }}
      ...
      {vids.length === 0 ? (
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "16px", padding: "24px" }}>
          ...
          {tab === "friends" && (
            <button
              style={{ background: T.pinkDim, border: `1px solid ${T.pinkBorder}`, borderRadius: T.r16, padding: "12px 20px", cursor: "pointer", fontFamily: T.fontDisplay, fontSize: "18px", color: T.pink, letterSpacing: "0.04em" }}
...
// ─────────────────────────────────────────────────────────────────
// ReelsFeed
// Touch events are attached directly via addEventListener (not React
// synthetic events) with passive:false on touchmove so we can call
// preventDefault() — this is the ONLY way to stop Android Chrome,
// Samsung Internet, Xiaomi, Oppo etc from stealing the scroll.
// ─────────────────────────────────────────────────────────────────
function ReelsFeed({ videos, user, navigate, onBadge }) {
  ...
  // Keep refs always current so event listeners don't get stale closures
  const idxRef = useRef(0);
  idxRef.current = idx;
  const totalRef = useRef(videos.length);
  totalRef.current = videos.length;
  const commentsRef = useRef(false);
  commentsRef.current = commentsOpen;

  // Touch tracking
  const t = useRef({ y: 0, x: 0, time: 0, dir: null, active: false });
  const lastSwipe = useRef(0);

  // ── Attach ALL touch listeners to the DOM element directly ─────
  // React synthetic events can't use passive:false which Android needs
  useEffect(() => {
    ...
    const onMove = e => {
      ...
      if (t.current.dir === null && (dy > 10 || dx > 10)) {
        t.current.dir = dy >= dx ? "vertical" : "horizontal";
      }

      // CRITICAL: only call preventDefault for vertical — stops Android
      // browser from triggering its own scroll / pull-to-refresh / back gesture
      if (t.current.dir === "vertical") {
        e.preventDefault();
      }
    };

    const onEnd = e => {
      if (!t.current.active || commentsRef.current) {
      ...
      const touch = e.changedTouches[0];
      const dy = t.current.y - touch.clientY; // positive = finger moved up = next video
      const dx = Math.abs(touch.clientX - t.current.x);
      const elapsed = Date.now() - t.current.time;

      // Thresholds tuned for Android:
      // - 45px minimum (works for slow drags and quick flicks)
      // - dx < 80 (not too horizontal)
      // - elapsed < 700ms (not a hold)
      // - 300ms debounce between swipes
      const valid = Math.abs(dy) > 45 && dx < 80 && elapsed < 700;
      if (!valid || Date.now() - lastSwipe.current < 300) return;
      lastSwipe.current = Date.now();

      if (dy > 0 && idxRef.current < totalRef.current - 1) {
        setIdx(p => p + 1);
      } else if (dy < 0 && idxRef.current > 0) {
        setIdx(p => p - 1);
      }
    };
    ...
    // passive: false on touchmove is essential — without it Android ignores preventDefault
  ...
  }, []); // empty deps — refs keep values current, no re-attach needed
  ...
  const handleWheel = useCallback(e => {
    ...
    if (e.deltaY > 30 && idxRef.current < totalRef.current - 1) {
      lastWheel.current = now; setIdx(p => p + 1);
    } else if (e.deltaY < -30 && idxRef.current > 0) {
      lastWheel.current = now; setIdx(p => p - 1);
    }
  }, []);
  ...
  return (
    <div
      ...
      style={{
        ...
        overflow: "hidden",
        position: "relative",
        // touchAction none tells ALL browsers (Samsung, Chrome, Firefox)
        // that JS is handling touch — no native scroll interference
        touchAction: "none",
        userSelect: "none",
        WebkitUserSelect: "none",
        // Prevent Samsung Internet's "quick menu" on long press
    ...
    >
      {videos.map((v, i) => (
        <ReelCard
          ...
          navigate={navigate}
          onBadge={onBadge}
          onCommentsChange={setCommentsOpen}
        />
      ))}
      ...
      <div style={{ position: "fixed", right: "8px", top: "50%", transform: "translateY(-50%)", display: "flex", flexDirection: "column", gap: "5px", zIndex: 50, pointerEvents: "none" }}>
        {videos.slice(Math.max(0, idx - 4), idx + 5).map((_, i) => {
          const a = Math.max(0, idx - 4) + i;
          return <div key={a} style={{ width: "3px", height: a === idx ? "18px" : "4px", borderRadius: "2px", background: a === idx ? T.pink : "rgba(255,255,255,0.15)", transition: "all 0.3s" }} />;
        })}
      </div>
    </div>
  );
...
function ReelCard({ video, user, isActive, offset, navigate, onBadge, onCommentsChange }) {
  ...
  return (
    <div style={{
      ...
      {/* Right actions — hidden when comments open */}
      {!showComments && (
      ...
      {!showComments && (
        <div style={{ position: "absolute", bottom: 0, left: 0, right: "60px", padding: "0 16px 90px", zIndex: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px", cursor: "pointer" }}
            onClick={() => navigate(`/profile/${video.uploadedBy}`)}>
            {video.uploaderPhoto
