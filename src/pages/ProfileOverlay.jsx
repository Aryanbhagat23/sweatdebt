...
function Overlay({ children, title, onBack }){
  return (
    // ✅ outermost div is CHALKBOARD so status bar area has no light gap/line
    <div style={{ minHeight:"100vh", background:CHALKBOARD }}>
      <div style={{
        minHeight:"100vh", background:MINT,
        paddingBottom:"90px",
      }}>
        {/* sticky chalkboard header — covers status bar with safe-area padding */}
        <div style={{
...
export default function ProfileOverlay({ user }){
  ...
  async function handleSaveCaption(){
    ...
    try{ await updateDoc(doc(db,"videos",editCaption.id),{ description: captionDraft.trim() }); }
  ...
  if(screen==="settings") return (
    <Overlay onBack={()=>setScreen("main")} title="Settings">
      ...
      <Card>
        ...
        {/* ✅ Fixed: no longer calls onClose() which doesn't exist */}
  ...
  if(screen==="social") return (
    <Overlay onBack={()=>setScreen("main")} title="Social Links">
      ...
      <button onClick={saveSocial} disabled={socialSaving} style={{
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:8000,display:"flex",alignItems:"flex-end"}}>
          <div style={{background:WHITE,width:"100%",borderRadius:"20px 20px 0 0",padding:"20px"}}>
            ...
            <textarea
              ...
              rows={3}
              style={{width:"100%",padding:"12px",borderRadius:"12px",border:`1.5px solid ${ACCENT}50`,fontFamily:T.fontBody,fontSize:"14px",color:CHALKBOARD,resize:"none",boxSizing:"border-box",outline:"none"}}
            />
            <div style={{display:"flex",gap:"10px",marginTop:"12px"}}>
              <button onClick={()=>setEditCaption(null)} style={{flex:1,padding:"12px",borderRadius:"12px",background:MINT,border:`1px solid ${ACCENT}40`,fontFamily:T.fontBody,fontSize:"14px",color:MUTED,cursor:"pointer"}}>Cancel</button>
              <button onClick={handleSaveCaption} style={{flex:1,padding:"12px",borderRadius:"12px",background:CHALKBOARD,border:"none",fontFamily:T.fontBody,fontSize:"14px",color:WHITE,cursor:"pointer",fontWeight:"600"}}>Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteConfirm && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:8000,display:"flex",alignItems:"flex-end"}}>
          <div style={{background:WHITE,width:"100%",borderRadius:"20px 20px 0 0",padding:"24px"}}>
            ...
            <p style={{fontFamily:T.fontBody,fontSize:"14px",color:MUTED,marginBottom:"20px"}}>This can't be undone. The video will be permanently removed.</p>
            <div style={{display:"flex",gap:"10px"}}>
              <button onClick={()=>setDeleteConfirm(null)} style={{flex:1,padding:"13px",borderRadius:"12px",background:MINT,border:`1px solid ${ACCENT}40`,fontFamily:T.fontBody,fontSize:"14px",color:MUTED,cursor:"pointer"}}>Cancel</button>
              <button onClick={()=>handleDeleteVideo(deleteConfirm)} style={{flex:1,padding:"13px",borderRadius:"12px",background:DANGER,border:"none",fontFamily:T.fontBody,fontSize:"14px",color:WHITE,cursor:"pointer",fontWeight:"600"}}>Delete</button>
            </div>
          </div>
        </div>
      )}
      ...
      {videoMenu && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:7000,display:"flex",alignItems:"flex-end"}} onClick={()=>setVideoMenu(null)}>
          <div onClick={e=>e.stopPropagation()} style={{background:WHITE,width:"100%",borderRadius:"20px 20px 0 0",overflow:"hidden"}}>
            ...
            <button onClick={()=>{setEditCaption({id:videoMenu.id,text:videoMenu.description||""});setCaptionDraft(videoMenu.description||"");setVideoMenu(null);}} style={menuBtnStyle}>✏️  Edit Caption</button>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"4px"}}>
          {videos.map(v=>(
            <div key={v.id} style={{position:"relative",aspectRatio:"9/16",background:"#000",borderRadius:"8px",overflow:"hidden"}}>
              ...
              <button onClick={e=>{e.stopPropagation();setVideoMenu(v);}} style={{
              ...
              {v.description && (
                <div style={{
                  ...
                  padding:"12px 6px 4px",fontFamily:T.fontBody,fontSize:"10px",color:WHITE,lineHeight:"1.3",
                }}>
  ...
  // ── MAIN SCREEN ───────────────────────────────────────────────────────────────
  return (
    // ✅ Outer wrapper is CHALKBOARD — this fills the status bar area so there's
    //    NO light line/gap at the very top of the screen on any phone.
    <div style={{ background:CHALKBOARD, minHeight:"100vh" }}>
      <div style={{
        minHeight:"100vh", background:MINT,
        ...
        <div style={{
          background:CHALKBOARD,
          // ✅ safe-area-inset-top accounts for notch/dynamic island/status bar
          paddingTop:"max(env(safe-area-inset-top, 0px), 48px)",
          paddingBottom:"20px",
          paddingLeft:"16px",
          paddingRight:"16px",
          ...
          {[20,50,80].map(y=>(
            <div key={y} style={{position:"absolute",left:0,right:0,top:`${y}%`,height:"1px",background:"rgba(255,255,255,0.04)"}}/>
          ))}

          <div style={{display:"flex",alignItems:"center",gap:"14px",position:"relative"}}>
            <div style={{position:"relative",flexShrink:0}}>
              <div style={{width:"68px",height:"68px",borderRadius:"50%",border:"3px solid rgba(255,255,255,0.25)",overflow:"hidden",background:"#1a3a2e",display:"flex",alignItems:"center",justifyContent:"center"}}>
                {photo
                  ? <img src={photo} alt="avatar" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                  : <span style={{fontSize:"28px"}}>🏃</span>
                }
              </div>
              <div style={{position:"absolute",bottom:"-4px",right:"-4px",background:tier.color,borderRadius:"50%",width:"22px",height:"22px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"12px",border:"2px solid "+CHALKBOARD}}>
                {tier.emoji}
              </div>
            </div>

            <div style={{flex:1}}>
              <div style={{fontFamily:T.fontDisplay,fontSize:"22px",letterSpacing:"0.04em",color:WHITE}}>{displayName}</div>
              {username && <div style={{fontFamily:T.fontMono||T.fontBody,fontSize:"12px",color:"rgba(255,255,255,0.55)",marginTop:"2px"}}>@{username}</div>}
              <div style={{display:"inline-flex",alignItems:"center",gap:"5px",marginTop:"6px",background:"rgba(255,255,255,0.12)",borderRadius:"20px",padding:"3px 10px"}}>
                ...
                <span style={{fontFamily:T.fontBody,fontSize:"12px",color:"rgba(255,255,255,0.5)"}}>· {profile.honourScore||0} pts</span>
              </div>
            </div>
          </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"6px",marginTop:"16px"}}>
            {[
              ...
              <div key={s.label} style={{background:"rgba(255,255,255,0.1)",borderRadius:"10px",padding:"8px 4px",textAlign:"center"}}>
        ...
        {Object.values(socialLinks).some(Boolean) && (
          <div style={{margin:"0 16px 12px",display:"flex",gap:"8px",flexWrap:"wrap"}}>
            {SOCIAL_PLATFORMS.filter(p=>socialLinks[p.key]).map(p=>(
              ...
              )} style={{background:WHITE,border:`1px solid ${ACCENT}30`,borderRadius:"20px",padding:"6px 12px",display:"flex",alignItems:"center",gap:"5px",cursor:"pointer"}}>
        ...
        <p style={{fontFamily:T.fontBody,fontSize:"11px",color:MUTED,textAlign:"center",padding:"8px 16px 0"}}>
