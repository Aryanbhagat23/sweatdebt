...
export default function Leaderboard({ user }) {
  ...
  useEffect(() => {
    ...
    const build = () => {
      ...
      usersData.forEach(u => {
        map[u.id] = {
          uid: u.id, name: u.displayName || u.name || "Unknown",
          username: u.username || "", photo: u.photoURL || u.photo || null,
          honour: typeof u.honour === "number" ? u.honour : 100,
          wins: 0, losses: 0, total: 0, sports: {}, fastestDebt: null,
        };
      });
      betsData.forEach(b => {
        if (b.createdBy && !map[b.createdBy]) {
          map[b.createdBy] = { uid:b.createdBy, name:b.createdByName||"Unknown", username:"", photo:null, honour:100, wins:0, losses:0, total:0, sports:{}, fastestDebt:null };
        }
      });
      betsData.forEach(b => {
        const u = map[b.createdBy]; if (!u) return;
        u.total++;
        if (b.status === "won")  u.wins++;
        if (b.status === "lost") u.losses++;
        if (b.sport) u.sports[b.sport] = (u.sports[b.sport]||0)+1;
        if (b.status === "lost" && b.proofUploaded && b.lostAt?.toDate && b.proofUploadedAt?.toDate) {
          const ms = b.proofUploadedAt.toDate() - b.lostAt.toDate();
          if (ms > 0 && (u.fastestDebt === null || ms < u.fastestDebt)) u.fastestDebt = ms;
        }
      });
      ...
      setRankings(ranked);
      setMyBets(betsData.filter(b => b.createdBy === user?.uid));
      setLoading(false);
    };

    const u1 = onSnapshot(collection(db,"bets"),   snap => { betsData  = snap.docs.map(d=>({id:d.id,...d.data()})); betsReady  = true; build(); });
    const u2 = onSnapshot(collection(db,"users"),  snap => { usersData = snap.docs.map(d=>({id:d.id,...d.data()})); usersReady = true; build(); });
    return () => { u1(); u2(); };
  }, [user]);
  ...
  const myWon   = myBets.filter(b=>b.status==="won").length;
  const myLost  = myBets.filter(b=>b.status==="lost").length;
  const myTotal = myBets.length;
  const myRate  = myTotal > 0 ? Math.round((myWon/myTotal)*100) : 0;
  const favSport = (() => {
    const c = {}; myBets.forEach(b=>{ if(b.sport) c[b.sport]=(c[b.sport]||0)+1; });
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.page, paddingBottom:"90px" }}>
      ...
      <div style={{ padding:"52px 16px 16px" }}>
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:"12px" }}>
          <div>
            ...
            <div style={{ fontFamily:T.fontBody, fontSize:"13px", color:C.muted, marginTop:"2px" }}>Who's sweating the most this season?</div>
          </div>
      ...
      {/* ── MY RANK CARD with personal stats ── */}
      {myEntry && (
        <div style={{ margin:"0 16px 16px", background:C.heading, borderRadius:"18px", padding:"16px", boxShadow:`0 2px 12px rgba(5,46,22,0.15)` }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"14px", marginBottom:"14px" }}>
            ...
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontFamily:T.fontBody, fontSize:"15px", fontWeight:"600", color:"#fff" }}>You — {myEntry.name}</div>
              <div style={{ fontFamily:T.fontMono, fontSize:"12px", color:`${C.accent}cc`, marginTop:"2px" }}>
                {myWon}W · {myLost}L · {myRate}% win rate
              </div>
            </div>
            <div style={{ textAlign:"right", flexShrink:0 }}>
              <div style={{ fontFamily:T.fontDisplay, fontSize:"28px", color:C.accent, letterSpacing:"0.04em" }}>{val(myEntry)}</div>
              <div style={{ fontFamily:T.fontMono, fontSize:"10px", color:`${C.accent}99`, letterSpacing:"0.08em", textTransform:"uppercase" }}>{valLabel}</div>
            </div>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px" }}>
            {[
              ...
              { label:"WIN STREAK",  val:`${myWon} 🔥`,                       color:C.gold   },
              ...
              <div key={s.label} style={{ background:"rgba(16,185,129,0.1)", border:"1px solid rgba(16,185,129,0.2)", borderRadius:"10px", padding:"10px 12px" }}>
                <div style={{ fontFamily:T.fontMono, fontSize:"9px", color:`${C.accent}70`, letterSpacing:"0.1em", marginBottom:"4px" }}>{s.label}</div>
                <div style={{ fontFamily:T.fontDisplay, fontSize:"18px", color:s.color, letterSpacing:"0.04em" }}>{s.val}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ padding:"0 16px" }}>
          ...
          <div style={{ fontFamily:T.fontMono, fontSize:"11px", color:C.muted, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:"10px" }}>All players</div>
            ...
            {sorted.map((r, i) => {
              ...
              return (
                <div key={r.uid} style={{ display:"flex", alignItems:"center", gap:"12px", background:isMe?C.heading:C.card, border:`1.5px solid ${isMe?C.accent:C.border}`, borderRadius:"16px", padding:"12px 14px", boxShadow:isMe?`0 2px 8px ${C.accent}20`:"0 1px 3px rgba(0,0,0,0.04)" }}>
                  ...
                  <div style={{ textAlign:"right", flexShrink:0 }}>
                    <div style={{ fontFamily:T.fontDisplay, fontSize:"24px", color:isMe?C.accent:C.heading, letterSpacing:"0.04em" }}>{val(r)}</div>
                    <div style={{ fontFamily:T.fontMono, fontSize:"10px", color:isMe?`${C.accent}99`:C.muted }}>{valLabel}</div>
                  </div>
                </div>
              );
            })}
...
function Podium({ u, rank, height, color, val, label, isTop }) {
  ...
  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"4px" }}>
      {isTop && <div style={{ background:"rgba(16,185,129,0.15)", border:"1px solid rgba(16,185,129,0.4)", borderRadius:"12px", padding:"2px 10px", fontFamily:"monospace", fontSize:"10px", color:"#10b981", letterSpacing:"0.06em", marginBottom:"2px" }}>TOP</div>}
      ...
      {u.photo
        ...
        : <div style={{ width:sz, height:sz, borderRadius:"50%", background:"#fff", border:`2px solid ${color}`, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"monospace", fontSize:isTop?"22px":"16px", color:"#052e16" }}>{(u.name||"?").charAt(0)}</div>
      }
      <div style={{ fontFamily:"system-ui", fontSize:"12px", fontWeight:"600", color:"#052e16", maxWidth:"70px", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", textAlign:"center" }}>{u.name?.split(" ")[0]}</div>
      <div style={{ fontFamily:"system-ui", fontSize:"15px", color:"#10b981", letterSpacing:"0.04em" }}>{val}{label==="wins"?"W":label==="debts"?"L":""}</div>
