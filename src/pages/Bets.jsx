...
import { BetsSkeleton } from "../components/Skeleton";
...
export default function Bets({ user }) {
  ...
  useEffect(() => {
    // My bets — no orderBy to avoid index requirement, sort client-side
    const q1 = query(
      collection(db, "bets"),
    ...
    }, err => console.error("incomingBets error:", err));
  ...
  if (loading) return <BetsSkeleton />;
  ...
  return (
    <div style={S.page}>
      ...
      <button
        style={{ ...S.btn, background: "transparent", border: "1px solid #d4ff00", color: "#d4ff00", marginTop: "-10px", marginBottom: "16px" }}
      ...
      {allBets.length === 0 ? (
        <div style={S.center}>
          ...
          <div style={S.emptySubText}>
            {activeTab === "incoming"
              ? "When friends challenge you, they appear here"
        ...
        <div>
          {allBets
            .filter(b => b.status === "pending" || b.status === "active" || b.status === "proof_uploaded" || b.status === "disputed")
            .map(bet => (
              <BetCard
                key={bet.id}
                bet={bet}
          ...
          {allBets.filter(b => b.status === "won" || b.status === "lost").length > 0 && (
    ...
    <div style={{ ...S.card, ...(isIncoming && bet.status === "pending" ? S.cardHighlight : {}) }}>
      <div style={S.cardTop}>
        ...
        <div style={S.cardUser}>
          <div style={S.cardName}>
            {isIncoming
              ? <><span style={{ color: "#d4ff00", fontWeight: "700" }}>{bet.createdByName}</span> challenged you!</>
      ...
      {isIncoming && bet.status === "pending" && (
        <div style={S.actionBtns}>
          ...
          <div style={S.acceptNote}>Film yourself completing the forfeit and upload</div>
        </div>
      )}

      {proofUploaded && !isIncoming && (
        <div style={S.waitingMsg}>⏳ Waiting for opponent to approve...</div>
      )}

      {bet.proofUrl && (
        <button style={S.viewProofBtn} onClick={() => window.open(bet.proofUrl, "_blank")}>
          ▶ VIEW PROOF VIDEO
        </button>
      )}
    </div>
