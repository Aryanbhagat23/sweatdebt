...
async function pruneOldNotifications(userId) {
  try {
    ...
    const snap = await getDocs(
      query(collection(db,"notifications"), where("toUserId","==",userId))
    );
    const toDelete = snap.docs.filter(d => {
      const ts = d.data().createdAt?.toDate?.();
      return ts && ts < cutoff && d.data().read; // only delete READ old ones
    });
...
export default function Bets({ user }) {
  ...
  // Prune old notifications once on mount
  useEffect(() => {
    if (user) pruneOldNotifications(user.uid);
  }, [user]);
  ...
  return (
    <div style={{ minHeight:"100vh", background:T.bg0, paddingBottom:"90px" }}>
      ...
      <div style={{ padding:"52px 16px 16px", display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
        ...
        {/* Bell sits top-right, same as in Feed */}
      ...
      <div style={{ margin:"0 16px 10px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        ...
        <button style={{ background:"transparent", border:"none", fontFamily:T.fontMono, fontSize:"11px", color:T.pink, cursor:"pointer" }}
      ...
      <div style={{ padding:"0 16px 16px", display:"flex", flexDirection:"column", gap:"10px" }}>
        ...
        <button style={{ background:"transparent", border:`1px solid ${T.borderMid}`, borderRadius:T.r16, padding:"14px", fontFamily:T.fontDisplay, fontSize:"20px", letterSpacing:"0.06em", color:T.white, cursor:"pointer" }}
      ...
      {/* Pending banner */}
      {pending.length > 0 && (
        <div style={{ margin:"0 16px 16px", background:T.pinkDim, border:`1px solid ${T.pinkBorder}`, borderRadius:T.r16, padding:"14px 16px", display:"flex", alignItems:"center", gap:"10px", cursor:"pointer" }}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", margin:"0 16px 16px", background:T.bg1, borderRadius:T.r16, padding:"4px", border:`1px solid ${T.border}` }}>
        {[
          ...
          <div key={t.key} style={{ padding:"12px", textAlign:"center", borderRadius:"12px", cursor:"pointer", background:tab===t.key?T.gradPrimary:"transparent", fontFamily:T.fontBody, fontSize:"14px", fontWeight:"600", color:tab===t.key?"#fff":T.muted, transition:"all 0.2s" }}
      ...
      {loading ? (
        ...
        <div style={{ textAlign:"center", padding:"48px 20px" }}>
          ...
          <div style={{ fontFamily:T.fontDisplay, fontSize:"24px", color:T.muted, letterSpacing:"0.04em" }}>
            {tab==="challenges" ? "No challenges yet" : "No bets placed yet"}
          </div>
          <div style={{ fontFamily:T.fontBody, fontSize:"14px", color:T.dim, marginTop:"8px" }}>
            {tab==="challenges" ? "Ask friends to challenge you!" : "Place your first bet above"}
          </div>
        </div>
      ) : (
      ...
      {expanded && (
        <div style={{ borderTop:`1px solid ${T.border}`, padding:"16px" }}>
          ...
          <div style={{ display:"flex", gap:"10px" }}>
            {isLost && (
              <button style={{ flex:1, background:T.gradFire, border:"none", borderRadius:T.r16, padding:"14px", fontFamily:T.fontDisplay, fontSize:"20px", letterSpacing:"0.06em", color:"#fff", cursor:"pointer" }}
                onClick={() => navigate(`/upload/${bet.id}`)}>
                📹 UPLOAD PROOF
              </button>
            )}
            {bet.status==="pending" && isChallenge && (
              <button style={{ flex:1, background:T.gradPrimary, border:"none", borderRadius:T.r16, padding:"14px", fontFamily:T.fontDisplay, fontSize:"20px", letterSpacing:"0.06em", color:"#fff", cursor:"pointer" }}
                onClick={async () => {
            ...
            {bet.status==="pending" && isChallenge && (
              <button style={{ flex:1, background:"transparent", border:`1px solid ${T.borderMid}`, borderRadius:T.r16, padding:"14px", fontFamily:T.fontDisplay, fontSize:"20px", letterSpacing:"0.06em", color:T.muted, cursor:"pointer" }}
                onClick={async () => await updateDoc(doc(db,"bets",bet.id), { status:"declined" })}>
                ✗ DECLINE
              </button>
            )}
            {bet.status==="accepted" && !isChallenge && (
              <button style={{ flex:1, background:"transparent", border:`1px solid ${T.borderMid}`, borderRadius:T.r16, padding:"14px", fontFamily:T.fontDisplay, fontSize:"18px", color:T.orange, cursor:"pointer" }}
                onClick={() => navigate(`/upload/${bet.id}`)}>
                📹 Submit Proof
              </button>
            )}
